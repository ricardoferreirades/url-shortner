...
pub fn validate_url(url: &str, config: &ValidationConfig) -> Result<String, ValidationErrorType> {
    ...
    if trimmed_url.len() > config.max_url_length {
        return Err(ValidationErrorType::UrlTooLong(trimmed_url.len(), config.max_url_length));
    }

    if trimmed_url.len() < config.min_url_length {
        return Err(ValidationErrorType::UrlTooShort(trimmed_url.len(), config.min_url_length));
    }
    ...
    if parsed_url.host().is_none() {
        return Err(ValidationErrorType::InvalidUrl("URL must have a host".to_string()));
    }
    ...
    if let Some(host) = parsed_url.host() {
        ...
        if host_str == "localhost" || host_str.starts_with("127.") || host_str.starts_with("192.168.") {
...
pub fn validate_short_code(short_code: &str) -> Result<String, ValidationErrorType> {
    ...
    if trimmed.len() < 3 || trimmed.len() > 50 {
        return Err(ValidationErrorType::General(
            format!("Short code must be between 3 and 50 characters, got {}", trimmed.len())
        ));
    }
    ...
    if !trimmed.chars().all(|c| c.is_alphanumeric() || c == '-' || c == '_') {
        return Err(ValidationErrorType::General(
            "Short code can only contain alphanumeric characters, hyphens, and underscores".to_string()
        ));
    }
...
pub struct ShortenUrlRequest {
    #[validate(length(min = 10, max = 2048, message = "URL must be between 10 and 2048 characters"))]
...
mod tests {
    ...
    fn test_validate_url_empty() {
        ...
        assert!(matches!(validate_url("", &config), Err(ValidationErrorType::EmptyUrl)));
        assert!(matches!(validate_url("   ", &config), Err(ValidationErrorType::EmptyUrl)));
    }
    ...
    fn test_validate_url_invalid_scheme() {
        ...
        assert!(matches!(validate_url("javascript:alert('xss')", &config), Err(ValidationErrorType::MaliciousPattern(_))));
        assert!(matches!(validate_url("data:text/html,<script>alert('xss')</script>", &config), Err(ValidationErrorType::MaliciousPattern(_))));
        assert!(matches!(validate_url("ftp://example.com", &config), Err(ValidationErrorType::MaliciousPattern(_))));
        assert!(matches!(validate_url("ssh://example.com", &config), Err(ValidationErrorType::InvalidScheme(_))));
    }
    ...
    fn test_validate_url_too_long() {
        ...
        assert!(matches!(validate_url(&long_url, &config), Err(ValidationErrorType::UrlTooLong(_, _))));
    }
    ...
    fn test_validate_url_too_short() {
        ...
        assert!(matches!(validate_url("https://a.co", &config), Err(ValidationErrorType::UrlTooShort(_, _))));
    }
