use crate::application::dto::{
    requests::BulkExpirationUpdateRequest,
...
pub async fn bulk_expiration_update_handler<R, U, P>(
    ...
    P: crate::domain::repositories::PasswordResetRepository + Send + Sync + Clone,
{
    ...
    info!(
        "Received bulk expiration update request for {} URLs (user: {})",
        request.url_ids.len(),
        user.id
    );
    ...
    match app_state
        .url_service
        .batch_update_expiration(
            &request.url_ids,
            Some(request.expiration_date),
            Some(user.id),
        )
        .await
    {
        Ok(result) => {
            ...
            info!(
                "Bulk expiration update completed: {} successful, {} failed",
