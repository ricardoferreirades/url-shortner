use url_shortner::shortener::{ShortenUrlRequest, ShortenUrlResponse, generate_short_code};
...
fn test_url_shortener_integration() {
    ...
    for url in test_urls {
        ...
        assert_eq!(short_code.len(), 8, "Short code should be exactly 8 characters");
        assert!(short_code.chars().all(|c| c.is_ascii_hexdigit()), "Short code should be hexadecimal");
        ...
        let deserialized_response: ShortenUrlResponse = serde_json::from_str(&response_json).unwrap();
